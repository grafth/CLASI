...
function [Store_nci,Store_nciNoise,Store_coh,Store_cohNoise,Store_offset,Store_vel,range,time] = Processing_CLASI_2_Channel(PulseSelection,data_dir,FileNum)
%% Load Raw Radar Data
...
EndNoiseIndex=15;                   % To end the noise averageing bins for voltage bias
VpeakA=0.4;                          % The voltage rails of the ADC for channel A
VpeakB=0.8;                          % The voltage rails of the ADC for channel B

if PulseSelection=="long";
    decimation_factor = 5;          % Data collected at 100 MHz, decimate to 20 MHz
    ...
    PRF = 400;
    numPulses = 4;
    PulseFrontTail=6;               %DEG change from 3 to 6 putting the front tail of the pulse back, this is according to the software trigger in the code
    ...
    PRF = 2000;             %DEG this is wrong for save file
    numPulses = 20;
    PulseFrontTail=15;             % DEG this may need to be change putting the front tail of the pulse back, this is according to the software trigger in the code
...
[voltage,fnameraw] = loadRawData_loop(data_dir,decimation_factor,FileNum-2,FileNum-2,StartNoiseIndex,EndNoiseIndex,VpeakA,VpeakB);
%     Vout=voltage.Storevoltage(1:signalEnd,:);
%     voltage.Storevoltage=voltage.Storevoltage(1:signalEnd,:);

voltageA = voltage.StorevoltageA(1:2^14,:);
voltageB=zeros(2^14,size(voltageA,2));
voltageB(1:size(voltage.StorevoltageB,1),:) = voltage.StorevoltageB(1:size(voltage.StorevoltageB,1),:);

NFFT = size(voltageA,1);
Freq = (-Fs/2:Fs/NFFT:Fs/2-Fs/NFFT);
%% digital filtering
% [voltageAUnshifted]=DigitalFilter(voltageA,Freq,PulseSelection);
% voltageA1=circshift(voltageAUnshifted,PWindex/2+EndNoiseIndex);
% [voltageBUnshifted]=DigitalFilter(voltageB,Freq,PulseSelection);
% voltageB1=circshift(voltageBUnshifted,PWindex/2+EndNoiseIndex);

[voltageA,MaxFreqCheckA]=DigitalFilterKaiser(voltageA,Freq,PulseSelection);

[voltageB,MaxFreqCheckB]=DigitalFilterKaiser(voltageB,Freq,PulseSelection);

%% remove error at the end of every 4th pulse and set the start of the pulse to the zero range
fth_Pulse = 4:4:size(voltageA,2);

% DEG CAG is this 70? with everything connected at the end ?
% pulseBegin = 70;?
pulseBegin = find(voltageA >=.15,1,'first');%DEG Change Threshold it was .2
...
voltageA = circshift(voltageA,-1*(pulseBegin-PulseFrontTail));
voltageA = voltageA(1:raw_sampleSize-(pulseBegin-PulseFrontTail),:);

%voltageB(raw_sampleSize-6:raw_sampleSize,fth_Pulse) = voltageB(raw_sampleSize-13:raw_sampleSize-7,fth_Pulse);
voltageB = circshift(voltageB,-1*(pulseBegin-PulseFrontTail));
voltageB = voltageB(1:raw_sampleSize-(pulseBegin-PulseFrontTail),:);
...
nRFI = 71;%71; DEG change                    % Number of samples to group together
